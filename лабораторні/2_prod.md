...
3. Для новоствореного процесу задайте два технологічних параметра (рис.2.1(5) і рис.2.1(6)), які будуть приймати участь в контролі і управлінні процесом створення продукту.
...
3. Визначте значення параметрів для робочого центру (рис.2.2(3))
...
1.  Виберіть модуль Products, для зручності виберіть групування по Типу (рис.2.3(1)).
...
3. Валідуйте версії обох продуктів.
...
1.  Відкрийте модуль Оператор, виберіть Create Test operation. У вікні вибору операцій виділіть продукт `pasteurized milk XX` і натисніть Create (рис.2.6(1)).
2.  Виберіть заплановану операцію і натисніть кнопку Info (рис.2.6(2)).
3.  Перейдіть у вікно інформації про операції та подивіться зміст всіх розділів і вкладок (Comment, Bill of Material, Get & Use \...). За допомогою цього вікна можна отримати інформацію про продукт а також визначати деякі параметри. Пізніше ми більш детально їх розглянемо.
4.  Запустіть на виконання операцію по створенню продукту. Для введення кількості виробленого продукту існує кнопка `Register Quantity`, але вона не активна. Для активації такої можливості перейдіть в властивості робочого центру і виставте опцію в TRUE (рис.2.6(4)).
5.  Зупиніть виконання поточної операції. У вікні вибору зупинки вкажіть `Break OFF`.
6.  Повторно створіть нову тестову операцію по виробництву продукту `pasteurized milk XX`. На панелі команд активізується кнопка `Register Quantity` (рис.2.6(5)). Натиснувши на неї можна вказати кількість виробленого продукту (рис.2.6(6)).
7.  Зупиніть виробництво продукту.
...
1.  В обладнанні `EQ_XX_PAST1` створіть параметр `CMD_START`. Значення `Mode` вкажіть рівним `OnOperationStart` (рис.2.7(1)). 
2.  Визначте значення `Expression = 1` (рис.2.7(2)). Таким чином при старті операції в робочому центрі в OPC Server буде встановлюватися значення `PXX_A1.PAST1.CMD` рівним 1.
3.  Створіть параметр `CMD_STOP` щоб при стопі операції в робочому центрі в OPC Server встановлюватися значення `PXX_A1.PAST1.CMD` рівним 2 (рис.2.7(3)).
4.  Відкрийте модуль оператора, створіть нову тестову операцію по виробництву продукту «pasteurized milk» і запустіть її на виконання. При старті з\'явиться вікно підтвердження відправки значення 1 в параметр `CMD_START` устатковання (рис.2.7(4)). При натисканні `Send`, значення будуть відправлені в OPC Server. Після чого з\'явиться вікно звіту відправки (рис.2.7(5)). Використовуючи UA Expert упевніться, що значення змінної `PXX_A1.PAST1.STA` змінилося в 2, що значить виконання.
...
**До уваги адміністраторів!** Для виконання даного завдання параметр `Possible product types to order` (MEScontrol.MES.Server-\> Scheduler) має дорівнювати `SalesUnit, IntermediateProduct, ConsumerUnit`.
...
2.  Використовуючи кнопку `Add` (рис.2.8(1)) додайте клієнтське замовлення (`Customer Order`).
...
4.  Тепер з клієнтського замовлення було зроблено одне робоче замовлення (`Work Orders`), оскільки для приготування напівфабрикату використовується тільки один процес `Pasteurization Milk XX` (рис.2.9(3)).
...
8.  Запустіть і через кілька секунд зупиніть виконання операції.
