...
**До уваги викладачів!** Для виконання даного завдання параметр `Possible product types to order` (`SystemConfiguration->Momentum.MES.Serverices-> Scheduler`) має дорівнювати `SalesUnit, IntermediateProduct, ConsumerUnit`.
...
5. Розкажіть як на основі параметрів рецепту задається необхідна кількість вхідної речовини та розраховується (або вказується) кількість вихідної?
