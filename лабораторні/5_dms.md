...
1) У модулі `WorkFlows` клонуйте (`Контекстне меню дії->Clone`) перед цим створений `Activity` і дайте йому назву `StopActiveOperation_XX` (де XX - номер Вашого варіанту). На вкладці `Definition` означте `Transactional = TURE`.
...
2. Розкажіть як проводиться означення записування значень параметру в DMS устатковання в робочому центрі.
...
5. Які блоки Workflow і для чого використовувалися в даній лабораторній роботі? 
