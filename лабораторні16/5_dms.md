...
В цьому і в наступному пункті необхідно конфігурувати ведення журналу для параметра `TT1` обладнання «EQ\_PAST1». Це потрібно, наприклад для формування звіту по температурі пастеризації. Для цього необхідно створити пакет, в якому налаштувати поля записів в ньому. 
...
2.  Додайте порожній пакет: add empty package (рис.5.1(1)). Створиться пакет з ім\'ям «New Package»
...
2. Додайте нову підписку в `Data Center-> Loggings -> Add Datacenter Logging` (рис.5.2(1))
...
6. Вкажіть назву підписки `TT1_XX Subscription` а також ім\'я параметра `TT101` устатковання «EQ\_XX\_PAST1» (рис.5.2(3)). Решту полів залиште без зміни. Таким чином в підписці братиме участь параметр устатковання, зміна якого буде ініціювати запис в пакет DMS.
...
3. Кожне значення записується не рідше ніж раз на хвилину і не частіше 2-х раз за хвилину, так як це вказується в налаштуваннях пакета. Використовуючи OPC UA Expert переведіть режим імітації в `smOn=False` змініть кілька разів значення `P1_A1.PAST1` в проміжках порядку 20-40 секунд. За відмітками часу записів подивіться, як часто пишеться значення в пакет DMS. Для поновлення перегляду записів користуйтеся кнопкою `Refresh`.

4. У модулі `Data Mining` виберіть `TT1 Log Package XX` і натисніть `Edit Work Center DMS Package View` (рис.5.3(3)). Додайте один View (рис.5.3(4)) і Workcenter, де він буде відображатися. Відкрийте вікно Operator Work Center `WC_XX_PAST1`. Перейдіть в \"Info\" -\> Data Mining (рис.5.3(5)). Ви побачите зміст пакета.
...
1.  Відкрийте модуль `DataMining` і створіть новий `Empty Packet`.
...
7.На вкладці `Definition` визначте ім\'я Activity = `Message XX` (де XX - номер варіанту), `Server = MES`, `Transactional = FALSE` (рис.5.4(6)).

8.Збережіть Activity кнопкою `Save` (рис.5.4(7)).
...
9.Додайте одну Variable з назвою «LoggerName» (рис.5.4(8)). Зверніть увагу що поле вводу змінних з'явиться тільки після кліку в області Workflow. В поле default записано значення за замовчуванням в форматі VB. 

10.Добавте з панелі елемент `WritePackageToDms` і з'єднайте його з елементом `Start`(рис.5.4(9)). Заповніть поля, як показано на рисунку.

Елемент `WritePackageToDms` робить запис у вказаний пакет Dms. У параметрах вказується зо саме треба писати. Поля `Value` в Workflow заповнюються в форматі виразів VB.NET. Там можна використовувати константи, змінні або викликати функції, об'єднуючи це все у вирази. 
...
- `InvokeServiceMethod` - запускає вказаний метод вказаного сервісу Momentum. У даному випадку використовується метод `GetActiveOperation` сервісу `OperationService` для отримання активної операції (та, яка виконується) за вказаним робочим центром
- `InvokeMethod` - виконує вказаний метод вказаного об'єкту    
...
2.  Визначте Arguments (рис.5.5(1)): `WC_Name` визначає ім\'я робочого центру, в якому потрібно зупинити активну (в роботі) операцію. 
3.  Визначте Variables (рис.5.5(1)): `WC` - необхідний робочий центр, `Oper` - необхідна операція. Для вибору типу змінної в списку типів треба вибрати `Browse of types` після чого задати фільтр, наприклад `WorkCenter` 
...
5.  У вузлі визначте умову `WC_Name = ""` це дає можливість визначити чи задано ім\'я робочого центру (рис.5.5(3)). Якщо ім\'я задано проводиться пошук робочого центру по імені, якщо немає - використовується ім\'я `WC_PAST1`. Замість `WC_PAST1` запишіть тут назву Вашого робочого центру. 
...
7.Доповніть програму іншими блоками, як це показано на рис.5.5(6). Не забувайте вказувати назви відповідно до Вашого варіанту. Налаштування блоку Decision відбувається через панель властивостей `Properties`. У повідомлення можна вписувати на кирилиці, наприклад:
...
1\. У модулі Layout створіть новий Subscriber (рис.5.7(1))
...
5\. Визначте `Variables` і `Arguments` як показано на рис.5.7(5) і рис.5.7(6). Значення `Default Values` будуть використовуватися для налагодження Subscriber.

6\. Створіть FlowChart, як показано на рис.5.7(7). Замість `StopActiveOperation` вкажіть створений раніше Workflow `StopActiveOperation_XX` (де XX - номер Вашого варіанту).

7\. Використовуючи кнопку «Create test operation» панелі інструментів Operator Control для Workcenter `WC_XX_PAST1` додайте Operation для продукту з кодом `IP_XX_101` і запустіть його на виконання.

8\. Змініть `Default Value` для arguments STA (рис.5.7(6)) в будь-яке значення крім 8, 10, 12. Збережіть створену Workflow, після чого запустіть її в тестовому режимі безпосередньо з редактора. Операція не повинна завершитися, так як STA відмінна від 8,10 і 12.
...
### 8. Створення і перевірка роботи Subscription для STA Past                           
 ...
 ![рис.5.8.](5media/8.png)

1. У модулі `Layout` відкрийте `DataCenter`.
...
6. Використовуючи UAExpert, змініть значення тега `PXX_A1.PAST1.STA` в значення відмінне від 8,10 і 12.
...
9. Використовуючи UAExpert, змініть значення тега `PXX_A1.PAST1.STA`в значення відмінне від 8,10 і 12.

---

### 9. Налаштування автоматичної реєстрації продукту на пастеризаторі.

У минулих лабораторних роботах реєстрація продукту проводилася вручну оператором. Однак при інтегруванні MOM з АСКТП, значення виробленого продукту доцільно брати з лічильників, ваг та інших реєстраторів. Для цього в Momentum передбачена функція Quantity Registration яка передбачає автоматичну реєстрацію якісного (Actual) і неякісного (Rejected) продукту за показами параметрів устатковання. Для такої реєстрації  передбачені відповідні предустановлені пакети DMS, які створюються автоматично.  У налаштуваннях реєстрації можна вказати як абсолютну (mode=Absolute з абсолютних лічильників) так і інкрементну (mode=Incremental з добавленням приросту до попереднього значення) реєстрацію   

У цьому прикладі необхідно сконфігурувати автоматичну реєстрацію якісного продукту за показаннями кількості продукту з лічильника-витратоміра обладнання `PAST1_FIQ1`.

![рис.5.9.](5media/9.png)

1\. Переконайтеся що для `WC_XX_PAST1` немає активних і запланованих операцій. Якщо такі є запустіть і зупиніть операції.

2\. У `WC_XX_PAST1` додайте Runtime Configuration (рис.5.9(1))

3\. У налаштуваннях `Actual Quantity` (рис.5.9(2)) виставте всі налаштування відповідно рис.5.9(3).
...
7\. Використовуючи кнопку `Create test operation` панелі інструментів Operator Control для Workcenter `WC_XX_PAST1` додайте Operation для продукту з кодом `IP_XX_101`. Запустіть на виконання.

8\. Для перевірки роботи автоматичної реєстрації запустіть клієнт OPC UA Expert. Переведіть установку в режим відключення імітації `smOn=false`. Змініть значення `PXX_A1_PAST1` на 3000. Переконайтеся що кількість виробленого продукту змінилося відповідної.

9\. Зупиніть тестову операцію. 
...
1\. Створіть для робочого центру `WC_XX_T101` Equipment `EQ_XX_T101` а також параметри, показані на рис.5.10(1).

2\. Створіть для робочого центру `WC_XX_PK105` Equipment `EQ_XX_PK105` а також параметри, показані на рис.5.10(2).
